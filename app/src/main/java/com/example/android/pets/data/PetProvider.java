...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        int gender = values.getAsInteger(PetContract.PetEntry.COLUMN_PET_GENRE);
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if(values.containsKey(PetContract.PetEntry.COLUMN_PET_BREED)){
            if(values.getAsString(PetContract.PetEntry.COLUMN_PET_BREED) == null){
                throw new IllegalArgumentException("Pet required breed");
            }
        }
        if(values.containsKey(PetContract.PetEntry.COLUMN_PET_GENRE)){
            if(values.getAsInteger(PetContract.PetEntry.COLUMN_PET_GENRE) != PetContract.PetEntry.GENDER_FEMALE && values.getAsInteger(PetContract.PetEntry.COLUMN_PET_GENRE) != PetContract.PetEntry.GENDER_MALE && values.getAsInteger(PetContract.PetEntry.COLUMN_PET_GENRE) != PetContract.PetEntry.GENDER_UNKNOWN){
                throw new IllegalArgumentException("Pet required a valid Genre");
            }
        }
        if(values.containsKey(PetContract.PetEntry.COLUMN_PET_WEIGHT)){
            if(values.getAsInteger(PetContract.PetEntry.COLUMN_PET_WEIGHT) <=0 ){
        ...
        int rowUpdate = database.update(PetContract.PetEntry.TABLE_NAME,values,selection,selectionArgs);
